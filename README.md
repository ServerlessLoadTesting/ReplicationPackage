...
```
docker cp airlinebooking:/ReplicationPackage/results .
```
...
* `RQ3_data_preprocess.py`: Generates the preprocessed `longitudinal_data.csv` from the raw data. Due to the large size exceeding the limitations of both CodeOcean and Zenodo, the raw data is available upon request from the first author.
* `RQ3_plot_overview.py`: Generates Figure 5 from the paper and saves it to `longitudinal_logscale.pdf`.
* `RQ3_breakpoints.py`: Conducts the changepoint analysis presented in Figure 6 and saves it as 10 pdf files with the naming scheme `breakpoints_WORKLOADCLASS.pdf`.
* `RQ3_sameday_montecarlo.py`: Conducts the monte-carlo simulation comparing the variation between measurements conducted on the same day and measurements conducted on different days. Results are saved to the file `same-day-monte-carlo.csv` and contain the information presented in Table 4 from the paper.
* `RQ3_differences_montecarlo.py`: Conducts the second monte carlo simulation presented in the paper in Table 5. Results are saved to the file `longitudinal_differences_montecarlo`.
